...
int FindDrive(void)
{
	...
	puts("Attempting to read sector 0\n");
	...
	if(partitioncount)
	{
		...
		unsigned int sig=mbr->Signature;
		printf("Partition signature is %d\n",sig);
		...
		if (!sd_read_sector(boot_sector, sector_buffer)) // read descriptor
		    return(0);
...
int GetCluster(int cluster)
{
    ...
    {
        if (!sd_read_sector(fat_start + sb,(char *)&fat_buffer))
            return(0);
